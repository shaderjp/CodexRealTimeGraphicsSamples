...
| Miss | ray が何にも当たらないとき sky color を返す |
...
GI 版では、primary hit から diffuse 方向へ 1 本だけ secondary ray を飛ばします。

```text
primary ray
  -> closest hit
    -> direct lighting + shadow
    -> cosine weighted hemisphere で方向を選ぶ
    -> secondary ray
      -> hit した先の lighting を読む
    -> indirect として足す
```

このサンプルは path tracer 全体を作るものではありません。目的は、1spp の diffuse 1-bounce を temporal accumulation で少しずつ収束させることです。

![BistroExteriorRaytracing GI D3D12 Final](images/BistroExteriorRaytracing%20GI%20D3D12%202026_05_06%2013_30_29.png)

### Temporal accumulation

GI は 1 frame に 1 sample だけだとノイズが多いです。そのため、前フレームまでの結果を accumulation texture に保持し、新しい sample と混ぜます。
...
3. `BistroExteriorRaytracingShared.hlsli`
   shader の構造体、payload、surface reconstruction、material shading を見る。
...
8. `BistroExteriorRaytracingGI.hlsl`
   GI と temporal accumulation の define を見る。
...
このサンプルは、その流れを Bistro という実シーンで確認するための教材になっています。
