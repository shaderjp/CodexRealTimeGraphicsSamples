...
前段の [Bistro 実装前までの振り返り](pre_bistro_implementation_review.ja.md) では、Triangle、Cube3D、SciFiHelmet、Skinning、ImGuiLighting までを扱いました。Bistro では、そこまでに作った要素を大きな実シーンへ広げ、さらにデバッグ UI、Shadow Map、Clustered Forward、Mesh Shader、Raytracing、Pathtracing へ展開しています。
    ...
    -> SciFiHelmet
      -> ImGuiLighting
        -> BistroExterior
          ...
          -> BistroExterior Clustered Forward
...
| `Samples/BistroExteriorClusteredForward` | emissive / procedural proxy light を使う多光源ラスタライズ版 |
...
最初に読むなら、まず `Samples/BistroExterior` の通常版だけで十分です。Shadow、Clustered Forward、Mesh Shader、Raytracing、Pathtracing は、そのあとに「同じシーンを別のテクニックで描く」派生として読むと理解しやすくなります。
...
![BistroExteriorClusteredForward Vulkan](<images/BistroExteriorClusteredForward Vulkan 2026_05_08 23_47_41.png>)

`BistroExteriorClusteredForward` は、Bistro のラスタライズ構成を保ったまま、emissive material / emissive texture と procedural proxy から local light を作り、多光源 forward と clustered forward を比較するサンプルです。

Clustered Forward 版だけをより細かく読みたい場合は、専用の [Bistro Clustered Forward 実装の振り返り](bistro_clustered_forward_implementation_review.ja.md) も参照してください。
...
- emissive surface を raster 向けの sphere light proxy に変換する。
- pixel shader で active light を直接ループする ManyLights 版と比較する。
- compute shader で screen tile と depth slice ごとの light list を作る。
- cluster record / light index buffer を pixel shader から読む。
- `Cluster Light Count`、`Cluster Slice`、`Cluster Overflow` で light assignment を確認する。
- directional shadow map を local-light 追加後の forward pass に統合する。

### 入門者向けの読み方

このサンプルは Mesh Shader や Raytracing より前に読むと分かりやすいです。まだ scene visibility は通常の rasterization のままで、変わったのは local light をどう選ぶかだからです。

```text
BistroExterior
  -> 1 directional light

BistroExteriorManyLights
  -> directional light + all active local lights

BistroExteriorClusteredForward
  -> directional light + cluster に入った local lights
```

まず `Local Light Contribution` debug view で local light の寄与だけを見て、次に `Cluster Light Count` で cluster assignment を見ます。Final が暗い、または light が効かないときは、light list、cluster list、pixel shader の参照先を順に分けて確認できます。
...
| 観点 | D3D12 | Vulkan |
| --- | --- | --- |
| cluster build | compute PSO | compute pipeline |
| cluster buffers | UAV / SRV structured buffers | storage buffers |
| compute-to-graphics sync | UAV barrier / resource barrier | buffer memory barrier |
| shadow depth | DSV + SRV | depth image + sampled image layout |
| debug readback | readback buffer | host-visible staging buffer |
...
まず `BistroExterior` で、実データを読み込み、D3D12 / Vulkan の見た目をそろえるための土台を作りました。次に Shadow Map で multi-pass rendering を加え、Clustered Forward で多光源と compute-driven light list を同じラスタライズの流れに足しました。その後、同じ Bistro scene を Mesh Shader、Raytracing、Pathtracing の教材として再利用できる形に広げました。
