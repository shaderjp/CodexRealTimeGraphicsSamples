...
namespace
{
    ...
    bool IsMeshletVisible(const Bistro::MeshletBounds& bounds, const XMFLOAT4 frustumPlanes[6], const XMFLOAT3* cameraPosition)
    {
        for (uint32_t planeIndex = 0; planeIndex < 6; ++planeIndex)
        {
            ...
            const float distance = plane.x * bounds.sphere.x + plane.y * bounds.sphere.y + plane.z * bounds.sphere.z + plane.w;
        ...
        if (cameraPosition)
        {
            const XMFLOAT3 centerToCamera(bounds.sphere.x - cameraPosition->x, bounds.sphere.y - cameraPosition->y, bounds.sphere.z - cameraPosition->z);
            ...
            const float coneDistance = centerToCamera.x * bounds.coneAxis.x + centerToCamera.y * bounds.coneAxis.y + centerToCamera.z * bounds.coneAxis.z;
...
bool BistroExteriorMeshShaderShadowCullingVulkan::QueueFamilyIndices::IsComplete() const
{
    return graphicsFamily != UINT32_MAX && presentFamily != UINT32_MAX;
}

BistroExteriorMeshShaderShadowCullingVulkan::BistroExteriorMeshShaderShadowCullingVulkan(uint32_t width, uint32_t height, const wchar_t* title) :
    m_width(width),
    m_height(height),
    m_title(title)
{
...
BistroExteriorMeshShaderShadowCullingVulkan::~BistroExteriorMeshShaderShadowCullingVulkan()
{
    Cleanup();
}

int BistroExteriorMeshShaderShadowCullingVulkan::Run(HINSTANCE instance, int showCommand)
{
...
LRESULT CALLBACK BistroExteriorMeshShaderShadowCullingVulkan::WindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    BistroExteriorMeshShaderShadowCullingVulkan* app = reinterpret_cast<BistroExteriorMeshShaderShadowCullingVulkan*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
    if (message == WM_NCCREATE)
    {
        auto* createStruct = reinterpret_cast<CREATESTRUCT*>(lParam);
        app = reinterpret_cast<BistroExteriorMeshShaderShadowCullingVulkan*>(createStruct->lpCreateParams);
...
void BistroExteriorMeshShaderShadowCullingVulkan::InitWindow(HINSTANCE instance, int showCommand)
{
    ...
    windowClass.lpszClassName = L"BistroExteriorMeshShaderShadowCullingVulkanWindowClass";
...
void BistroExteriorMeshShaderShadowCullingVulkan::InitVulkan()
{
    LoadModel();
    CreateInstance();
    CreateSurface();
...
void BistroExteriorMeshShaderShadowCullingVulkan::MainLoop()
{
    MSG message{};
...
void BistroExteriorMeshShaderShadowCullingVulkan::Render()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::SetOutputResolution(uint32_t width, uint32_t height)
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::RecreateSwapChain()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::WaitIdle()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::Cleanup()
{
    if (m_device != VK_NULL_HANDLE)
    {
        vkDeviceWaitIdle(m_device);
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateInstance()
{
    ...
    {
        ThrowIfFailed(enumerateInstanceVersion(&instanceVersion), "Failed to query Vulkan instance version.");
    }
    if (instanceVersion < VK_API_VERSION_1_2)
    {
        throw std::runtime_error("Vulkan 1.2 or newer is required for this mesh shader sample.");
    }

    VkApplicationInfo appInfo{};
    appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    appInfo.pApplicationName = "BistroExteriorMeshShader Shadow Vulkan";
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateSurface()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::PickPhysicalDevice()
{
    ...
    std::ostringstream rejectionLog;
    rejectionLog << "No suitable Vulkan device was found.\n";
    rejectionLog << "Required: Vulkan 1.2, graphics+present queue, "
                 << VK_KHR_SWAPCHAIN_EXTENSION_NAME << ", " << VK_EXT_MESH_SHADER_EXTENSION_NAME
                 << ", meshShader and taskShader features.\n";
    ...
    for (VkPhysicalDevice device : devices)
    {
        ...
        {
            rejectionLog << " - missing graphics/present queue family\n";
        }
        ...
        {
            rejectionLog << " - missing device extension: " << missingExtension << "\n";
        }
        ...
        {
            rejectionLog << " - meshShader feature is not supported/enabled by the driver\n";
        }
        if (meshShaderFeatures.taskShader != VK_TRUE)
        {
            rejectionLog << " - taskShader feature is not supported/enabled by the driver\n";
        }
        ...
        if (indices.IsComplete())
        {
            ...
            {
                rejectionLog << " - no swapchain surface formats\n";
            }
            if (support.presentModes.empty())
            {
                rejectionLog << " - no swapchain present modes\n";
            }
        }
    }

    throw std::runtime_error(rejectionLog.str());
}

void BistroExteriorMeshShaderShadowCullingVulkan::CreateLogicalDevice()
{
    ...
    {
        throw std::runtime_error("VK_EXT_mesh_shader is present, but vkCmdDrawMeshTasksEXT could not be loaded.");
    }
}

void BistroExteriorMeshShaderShadowCullingVulkan::CreateSwapChain()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateImageViews()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateRenderPass()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateShadowRenderPass()
{
    VkAttachmentDescription depthAttachment{};
    depthAttachment.format = DepthFormat;
    depthAttachment.samples = VK_SAMPLE_COUNT_1_BIT;
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateDescriptorSetLayout()
{
    ...
    for (uint32_t i = 0; i < 5; ++i)
    {
        ...
        bindings[9 + i].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
        bindings[9 + i].descriptorCount = 1;
        bindings[9 + i].stageFlags = VK_SHADER_STAGE_TASK_BIT_EXT | VK_SHADER_STAGE_MESH_BIT_EXT;
    }
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateGraphicsPipeline()
{
    std::wstring shaderDirectory = GetExecutableDirectory();
    VkShaderModule taskShaderModule = CreateShaderModule(ReadFile(shaderDirectory + L"BistroExteriorMeshShaderShadowCulling.as.spv"));
    VkShaderModule meshShaderModule = CreateShaderModule(ReadFile(shaderDirectory + L"BistroExteriorMeshShaderShadowCulling.ms.spv"));
    VkShaderModule pixelShaderModule = CreateShaderModule(ReadFile(shaderDirectory + L"BistroExteriorMeshShaderShadowCulling.ps.spv"));
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateShadowPipeline()
{
    if (m_shadowPipeline)
    {
        vkDestroyPipeline(m_device, m_shadowPipeline, nullptr);
    ...
    std::wstring shaderDirectory = GetExecutableDirectory();
    VkShaderModule taskShaderModule = CreateShaderModule(ReadFile(shaderDirectory + L"BistroExteriorMeshShaderShadowCulling.as.spv"));
    VkShaderModule meshShaderModule = CreateShaderModule(ReadFile(shaderDirectory + L"BistroExteriorMeshShaderShadowCulling.shadow.ms.spv"));
    VkShaderModule pixelShaderModule = CreateShaderModule(ReadFile(shaderDirectory + L"BistroExteriorMeshShaderShadowCulling.shadow.ps.spv"));
    ...
    meshShaderStageInfo.stage = VK_SHADER_STAGE_MESH_BIT_EXT;
    meshShaderStageInfo.module = meshShaderModule;
    meshShaderStageInfo.pName = "MSShadowMain";
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateDepthResources()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateShadowResources()
{
    CreateImage(
        m_shadowResolution,
        m_shadowResolution,
...
void BistroExteriorMeshShaderShadowCullingVulkan::DestroyShadowResources()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::RecreateShadowResources()
{
    vkDeviceWaitIdle(m_device);
    DestroyShadowResources();
    CreateShadowResources();
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateFramebuffers()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateCommandPool()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::LoadModel()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateVertexBuffer()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateMeshletBuffers()
{
...
uint32_t BistroExteriorMeshShaderShadowCullingVulkan::CreateTextureResource(const std::wstring& path, bool srgb, const uint8_t fallback[4], std::map<std::wstring, uint32_t>& cache)
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateTextureImages()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateTextureSampler()
{
    VkSamplerCreateInfo samplerInfo{};
    samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
    samplerInfo.magFilter = VK_FILTER_LINEAR;
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateUniformBuffer()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateDescriptorPool()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateDescriptorSets()
{
    ...
    for (uint32_t materialIndex = 0; materialIndex < m_descriptorSets.size(); ++materialIndex)
    {
        ...
        VkDescriptorBufferInfo vertexBufferInfo{ m_vertexBuffer, 0, VK_WHOLE_SIZE };
        VkDescriptorBufferInfo meshletBufferInfo{ m_meshletBuffer, 0, VK_WHOLE_SIZE };
        VkDescriptorBufferInfo meshletVertexBufferInfo{ m_meshletVertexBuffer, 0, VK_WHOLE_SIZE };
        VkDescriptorBufferInfo meshletTriangleBufferInfo{ m_meshletTriangleBuffer, 0, VK_WHOLE_SIZE };
        VkDescriptorBufferInfo meshletBoundsBufferInfo{ m_meshletBoundsBuffer, 0, VK_WHOLE_SIZE };
        ...
        VkDescriptorBufferInfo* storageInfos[] =
        {
            &vertexBufferInfo,
            &meshletBufferInfo,
            &meshletVertexBufferInfo,
            &meshletTriangleBufferInfo,
            &meshletBoundsBufferInfo
        };
        for (uint32_t i = 0; i < 5; ++i)
        {
            descriptorWrites[9 + i].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrites[9 + i].dstSet = m_descriptorSets[materialIndex];
            descriptorWrites[9 + i].dstBinding = 9 + i;
            descriptorWrites[9 + i].descriptorCount = 1;
            descriptorWrites[9 + i].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
            descriptorWrites[9 + i].pBufferInfo = storageInfos[i];
        }
        vkUpdateDescriptorSets(m_device, static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
    }
}

void BistroExteriorMeshShaderShadowCullingVulkan::UpdateShadowDescriptorSets()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::InitializeImGui()
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
void BistroExteriorMeshShaderShadowCullingVulkan::ShutdownImGui()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::BuildUI()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::BuildRendererStatsUI()
{
    ...
    ImGui::Text("Meshlet Dispatches: %zu", m_scene.meshletDispatchRanges.size());
    ImGui::Text("Visible Meshlets: %u", m_lastShadowVisibleMeshlets);
    ImGui::Text("Culled Meshlets: %u", static_cast<uint32_t>(m_scene.meshlets.size()) - m_lastShadowVisibleMeshlets);
    ImGui::Text("Primitives: %llu", static_cast<unsigned long long>(primitiveCount));
...
void BistroExteriorMeshShaderShadowCullingVulkan::ResetLight()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::ResetCameraView()
{
    m_camera.Reset(m_defaultCameraPosition, m_defaultCameraYaw, m_defaultCameraPitch);
}

void BistroExteriorMeshShaderShadowCullingVulkan::ResetCameraSpeeds()
{
    m_baseMoveSpeed = 17.0f;
    m_fastMoveSpeed = 58.2f;
}

void BistroExteriorMeshShaderShadowCullingVulkan::ResetShadowSettings()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateCommandBuffers()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::RecordCommandBuffer(uint32_t imageIndex)
{
    ...
    for (const Bistro::MeshletDispatchRange& range : m_scene.meshletDispatchRanges)
    {
        ...
        {
            const uint32_t meshletCount = (std::min)(meshletsRemaining, 65535u * 128u);
            const uint32_t dispatchCount = (meshletCount + 127u) / 128u;
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateSyncObjects()
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::UpdateUniformBuffer()
{
    ...
    storeFrustumPlanes(viewProjection, constants.frustumPlanes);
    storeFrustumPlanes(lightViewProjection, constants.lightFrustumPlanes);
    ...
    for (const Bistro::MeshletBounds& bounds : m_scene.meshletBounds)
    {
        ...
        m_lastShadowVisibleMeshlets += IsMeshletVisible(bounds, constants.lightFrustumPlanes, nullptr) ? 1u : 0u;
    }
...
bool BistroExteriorMeshShaderShadowCullingVulkan::IsDeviceSuitable(VkPhysicalDevice device) const
{
    QueueFamilyIndices indices = FindQueueFamilies(device);
    if (!indices.IsComplete())
    {
...
BistroExteriorMeshShaderShadowCullingVulkan::QueueFamilyIndices BistroExteriorMeshShaderShadowCullingVulkan::FindQueueFamilies(VkPhysicalDevice device) const
{
...
BistroExteriorMeshShaderShadowCullingVulkan::SwapChainSupport BistroExteriorMeshShaderShadowCullingVulkan::QuerySwapChainSupport(VkPhysicalDevice device) const
{
...
VkSurfaceFormatKHR BistroExteriorMeshShaderShadowCullingVulkan::ChooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats) const
{
...
VkPresentModeKHR BistroExteriorMeshShaderShadowCullingVulkan::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& presentModes) const
{
...
VkExtent2D BistroExteriorMeshShaderShadowCullingVulkan::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) const
{
...
uint32_t BistroExteriorMeshShaderShadowCullingVulkan::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) const
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& memory)
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageUsageFlags usage, VkImage& image, VkDeviceMemory& memory)
{
...
VkImageView BistroExteriorMeshShaderShadowCullingVulkan::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) const
{
...
VkCommandBuffer BistroExteriorMeshShaderShadowCullingVulkan::BeginSingleTimeCommands() const
{
    VkCommandBufferAllocateInfo allocateInfo{};
    allocateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocateInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
...
void BistroExteriorMeshShaderShadowCullingVulkan::EndSingleTimeCommands(VkCommandBuffer commandBuffer) const
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::CopyBufferToImage(VkBuffer buffer, VkImage image, const Bistro::TextureData& texture) const
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::TransitionImageLayout(VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) const
{
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
std::vector<char> BistroExteriorMeshShaderShadowCullingVulkan::ReadFile(const std::wstring& path) const
{
...
VkShaderModule BistroExteriorMeshShaderShadowCullingVulkan::CreateShaderModule(const std::vector<char>& code) const
{
...
std::wstring BistroExteriorMeshShaderShadowCullingVulkan::GetExecutableDirectory() const
{
...
void BistroExteriorMeshShaderShadowCullingVulkan::ThrowIfFailed(VkResult result, const char* message) const
{
