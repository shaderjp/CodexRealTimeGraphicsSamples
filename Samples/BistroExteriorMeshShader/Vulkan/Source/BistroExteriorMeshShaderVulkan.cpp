...
bool BistroExteriorMeshShaderVulkan::QueueFamilyIndices::IsComplete() const
{
    return graphicsFamily != UINT32_MAX && presentFamily != UINT32_MAX;
}

BistroExteriorMeshShaderVulkan::BistroExteriorMeshShaderVulkan(uint32_t width, uint32_t height, const wchar_t* title) :
    m_width(width),
    m_height(height),
    m_title(title)
{
...
BistroExteriorMeshShaderVulkan::~BistroExteriorMeshShaderVulkan()
{
    Cleanup();
}

int BistroExteriorMeshShaderVulkan::Run(HINSTANCE instance, int showCommand)
{
...
LRESULT CALLBACK BistroExteriorMeshShaderVulkan::WindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    BistroExteriorMeshShaderVulkan* app = reinterpret_cast<BistroExteriorMeshShaderVulkan*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
    if (message == WM_NCCREATE)
    {
        auto* createStruct = reinterpret_cast<CREATESTRUCT*>(lParam);
        app = reinterpret_cast<BistroExteriorMeshShaderVulkan*>(createStruct->lpCreateParams);
...
void BistroExteriorMeshShaderVulkan::InitWindow(HINSTANCE instance, int showCommand)
{
    ...
    windowClass.lpszClassName = L"BistroExteriorMeshShaderVulkanWindowClass";
...
void BistroExteriorMeshShaderVulkan::InitVulkan()
{
    LoadModel();
    CreateInstance();
    CreateSurface();
...
void BistroExteriorMeshShaderVulkan::MainLoop()
{
    MSG message{};
...
void BistroExteriorMeshShaderVulkan::Render()
{
...
void BistroExteriorMeshShaderVulkan::SetOutputResolution(uint32_t width, uint32_t height)
{
...
void BistroExteriorMeshShaderVulkan::RecreateSwapChain()
{
...
void BistroExteriorMeshShaderVulkan::WaitIdle()
{
...
void BistroExteriorMeshShaderVulkan::Cleanup()
{
    if (m_device != VK_NULL_HANDLE)
    {
        vkDeviceWaitIdle(m_device);
...
void BistroExteriorMeshShaderVulkan::CreateInstance()
{
    VkApplicationInfo appInfo{};
    appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    appInfo.pApplicationName = "BistroExteriorMeshShader Vulkan";
...
void BistroExteriorMeshShaderVulkan::CreateSurface()
{
...
void BistroExteriorMeshShaderVulkan::PickPhysicalDevice()
{
    ...
    throw std::runtime_error("No suitable Vulkan device was found. BistroExteriorMeshShaderVulkan requires Vulkan 1.2, swapchain support, VK_EXT_mesh_shader, and meshShader feature support.");
}

void BistroExteriorMeshShaderVulkan::CreateLogicalDevice()
{
...
void BistroExteriorMeshShaderVulkan::CreateSwapChain()
{
...
void BistroExteriorMeshShaderVulkan::CreateImageViews()
{
...
void BistroExteriorMeshShaderVulkan::CreateRenderPass()
{
...
void BistroExteriorMeshShaderVulkan::CreateDescriptorSetLayout()
{
    ...
    for (uint32_t i = 0; i < 4; ++i)
    {
        ...
        bindings[7 + i].stageFlags = VK_SHADER_STAGE_MESH_BIT_EXT;
    }
...
void BistroExteriorMeshShaderVulkan::CreateGraphicsPipeline()
{
    std::wstring shaderDirectory = GetExecutableDirectory();
    VkShaderModule meshShaderModule = CreateShaderModule(ReadFile(shaderDirectory + L"BistroExteriorMeshShader.ms.spv"));
    VkShaderModule pixelShaderModule = CreateShaderModule(ReadFile(shaderDirectory + L"BistroExteriorMeshShader.ps.spv"));
...
void BistroExteriorMeshShaderVulkan::CreateDepthResources()
{
...
void BistroExteriorMeshShaderVulkan::CreateFramebuffers()
{
...
void BistroExteriorMeshShaderVulkan::CreateCommandPool()
{
...
void BistroExteriorMeshShaderVulkan::LoadModel()
{
...
void BistroExteriorMeshShaderVulkan::CreateVertexBuffer()
{
...
void BistroExteriorMeshShaderVulkan::CreateMeshletBuffers()
{
...
uint32_t BistroExteriorMeshShaderVulkan::CreateTextureResource(const std::wstring& path, bool srgb, const uint8_t fallback[4], std::map<std::wstring, uint32_t>& cache)
{
...
void BistroExteriorMeshShaderVulkan::CreateTextureImages()
{
...
void BistroExteriorMeshShaderVulkan::CreateTextureSampler()
{
...
void BistroExteriorMeshShaderVulkan::CreateUniformBuffer()
{
...
void BistroExteriorMeshShaderVulkan::CreateDescriptorPool()
{
    ...
    poolSizes[3].descriptorCount = static_cast<uint32_t>(m_scene.materials.size() * 4);
...
void BistroExteriorMeshShaderVulkan::CreateDescriptorSets()
{
    ...
    for (uint32_t materialIndex = 0; materialIndex < m_descriptorSets.size(); ++materialIndex)
    {
        ...
        std::array<VkDescriptorBufferInfo, 4> meshletBufferInfos = { vertexInfo, meshletInfo, meshletVertexInfo, meshletTriangleInfo };
...
void BistroExteriorMeshShaderVulkan::InitializeImGui()
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
void BistroExteriorMeshShaderVulkan::ShutdownImGui()
{
...
void BistroExteriorMeshShaderVulkan::BuildUI()
{
...
void BistroExteriorMeshShaderVulkan::BuildRendererStatsUI()
{
...
void BistroExteriorMeshShaderVulkan::ResetLight()
{
...
void BistroExteriorMeshShaderVulkan::ResetCameraView()
{
    m_camera.Reset(m_defaultCameraPosition, m_defaultCameraYaw, m_defaultCameraPitch);
}

void BistroExteriorMeshShaderVulkan::ResetCameraSpeeds()
{
    m_baseMoveSpeed = 17.0f;
    m_fastMoveSpeed = 58.2f;
}

void BistroExteriorMeshShaderVulkan::CreateCommandBuffers()
{
...
void BistroExteriorMeshShaderVulkan::RecordCommandBuffer(uint32_t imageIndex)
{
    ...
    for (const Bistro::MeshletDispatchRange& range : m_scene.meshletDispatchRanges)
    {
        ...
        {
            const uint32_t dispatchCount = (std::min)(meshletsRemaining, MaxMeshDispatchWorkGroupsX);
            ...
            meshletBase += dispatchCount;
            meshletsRemaining -= dispatchCount;
        }
    }
...
void BistroExteriorMeshShaderVulkan::CreateSyncObjects()
{
...
void BistroExteriorMeshShaderVulkan::UpdateUniformBuffer()
{
...
bool BistroExteriorMeshShaderVulkan::IsDeviceSuitable(VkPhysicalDevice device) const
{
...
BistroExteriorMeshShaderVulkan::QueueFamilyIndices BistroExteriorMeshShaderVulkan::FindQueueFamilies(VkPhysicalDevice device) const
{
...
BistroExteriorMeshShaderVulkan::SwapChainSupport BistroExteriorMeshShaderVulkan::QuerySwapChainSupport(VkPhysicalDevice device) const
{
...
VkSurfaceFormatKHR BistroExteriorMeshShaderVulkan::ChooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats) const
{
...
VkPresentModeKHR BistroExteriorMeshShaderVulkan::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& presentModes) const
{
...
VkExtent2D BistroExteriorMeshShaderVulkan::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) const
{
...
uint32_t BistroExteriorMeshShaderVulkan::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) const
{
...
void BistroExteriorMeshShaderVulkan::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& memory)
{
...
void BistroExteriorMeshShaderVulkan::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageUsageFlags usage, VkImage& image, VkDeviceMemory& memory)
{
...
VkImageView BistroExteriorMeshShaderVulkan::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) const
{
...
VkCommandBuffer BistroExteriorMeshShaderVulkan::BeginSingleTimeCommands() const
{
    VkCommandBufferAllocateInfo allocateInfo{};
    allocateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocateInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
...
void BistroExteriorMeshShaderVulkan::EndSingleTimeCommands(VkCommandBuffer commandBuffer) const
{
...
void BistroExteriorMeshShaderVulkan::CopyBufferToImage(VkBuffer buffer, VkImage image, const Bistro::TextureData& texture) const
{
...
void BistroExteriorMeshShaderVulkan::TransitionImageLayout(VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) const
{
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
std::vector<char> BistroExteriorMeshShaderVulkan::ReadFile(const std::wstring& path) const
{
...
VkShaderModule BistroExteriorMeshShaderVulkan::CreateShaderModule(const std::vector<char>& code) const
{
...
std::wstring BistroExteriorMeshShaderVulkan::GetExecutableDirectory() const
{
...
void BistroExteriorMeshShaderVulkan::ThrowIfFailed(VkResult result, const char* message) const
{
