...
namespace
{
    ...
    const std::array<const wchar_t*, ImGuiLightingD3D12::TextureCount> TextureNames =
    {
...
ImGuiLightingD3D12::ImGuiLightingD3D12(UINT width, UINT height, std::wstring name) :
    ...
    m_lightDirection{ -0.35f, -0.8f, 0.45f },
    m_lightColor{ 1.0f, 0.96f, 0.88f },
    m_lightIntensity(5.0f)
{
}

void ImGuiLightingD3D12::OnInit()
{
...
void ImGuiLightingD3D12::LoadPipeline()
{
...
void ImGuiLightingD3D12::LoadAssets()
{
    ...
    ThrowIfFailed(ReadDataFromFile(GetAssetFullPath(L"ImGuiLighting.vs.cso").c_str(), &vertexShaderData, &vertexShaderDataLength));
    ThrowIfFailed(ReadDataFromFile(GetAssetFullPath(L"ImGuiLighting.ps.cso").c_str(), &pixelShaderData, &pixelShaderDataLength));
...
void ImGuiLightingD3D12::OnUpdate()
{
    BuildUI();
    UpdateConstantBuffer();
}

void ImGuiLightingD3D12::OnRender()
{
    PopulateCommandList();
    ID3D12CommandList* commandLists[] = { m_commandList.Get() };
    m_commandQueue->ExecuteCommandLists(_countof(commandLists), commandLists);
...
void ImGuiLightingD3D12::OnDestroy()
{
    WaitForPreviousFrame();
    ShutdownImGui();
    if (m_constantBuffer)
...
void ImGuiLightingD3D12::LoadModel()
{
...
void ImGuiLightingD3D12::InitializeImGui()
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
void ImGuiLightingD3D12::ShutdownImGui()
{
...
void ImGuiLightingD3D12::BuildUI()
{
...
void ImGuiLightingD3D12::ResetLight()
{
...
ImGuiLightingD3D12::ImageData ImGuiLightingD3D12::LoadTexture(const std::wstring& path) const
{
...
void ImGuiLightingD3D12::CreateTexture(UINT index, const ImageData& image)
{
...
void ImGuiLightingD3D12::PopulateCommandList()
{
...
void ImGuiLightingD3D12::WaitForPreviousFrame()
{
...
void ImGuiLightingD3D12::UpdateConstantBuffer()
{
    ...
    m_constantBufferData.lightDirection = XMFLOAT4(normalizedDirection.x, normalizedDirection.y, normalizedDirection.z, 0.0f);
    m_constantBufferData.lightColor = XMFLOAT4(m_lightColor[0], m_lightColor[1], m_lightColor[2], m_lightIntensity);
    memcpy(m_mappedConstantBuffer, &m_constantBufferData, sizeof(m_constantBufferData));
}

void ImGuiLightingD3D12::AllocateImGuiDescriptor(ImGui_ImplDX12_InitInfo* info, D3D12_CPU_DESCRIPTOR_HANDLE* outCpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE* outGpuHandle)
{
    auto* sample = static_cast<ImGuiLightingD3D12*>(info->UserData);
...
void ImGuiLightingD3D12::FreeImGuiDescriptor(ImGui_ImplDX12_InitInfo*, D3D12_CPU_DESCRIPTOR_HANDLE, D3D12_GPU_DESCRIPTOR_HANDLE)
{
}
