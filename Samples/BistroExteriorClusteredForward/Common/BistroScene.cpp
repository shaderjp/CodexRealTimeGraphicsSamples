...
namespace
{
    ...
    void PushSphereProxy(std::vector<Bistro::RasterLight>& lights, const XMFLOAT3& position, float area, const XMFLOAT3& radiance, float sourceType)
    {
        const float radius = std::clamp(std::sqrt((std::max)(area, 0.0001f)) * 4.0f, 1.25f, 16.0f);

        Bistro::RasterLight light{};
        light.positionRadius = XMFLOAT4(position.x, position.y, position.z, radius);
        light.radianceSource = XMFLOAT4(radiance.x, radiance.y, radiance.z, sourceType);
        lights.push_back(light);
    }

    void PushRectProxy(std::vector<Bistro::RasterLight>& lights, const XMFLOAT3& center, const XMFLOAT3& edge0, const XMFLOAT3& edge1, const XMFLOAT3& radiance)
    {
        const float area = (std::max)(Length(Cross(edge0, edge1)), 0.0001f);
        PushSphereProxy(lights, center, area, radiance, 1.0f);
    }
...
namespace Bistro
{
    ...
    LightBuildResult BuildRasterLightList(const Scene& scene, uint32_t maxEmissiveTriangleLights)
    {
        LightBuildResult result;
        std::vector<RasterLight> emissiveCandidates;
        ...
        for (const DrawItem& draw : scene.draws)
        {
            ...
            for (uint32_t i = 0; i + 2 < draw.indexCount; i += 3)
            {
                ...
                const XMFLOAT3 centroid = Scale(Add(Add(p0, p1), p2), 1.0f / 3.0f);
                PushSphereProxy(emissiveCandidates, centroid, area, radiance, 0.0f);
            }
        }
        ...
        result.emissiveProxyCount = static_cast<uint32_t>(result.lights.size());
        ...
        PushRectProxy(result.lights, XMFLOAT3(center.x - extent.x * 0.17f, windowY, frontZ), XMFLOAT3(windowWidth, 0.0f, 0.0f), XMFLOAT3(0.0f, windowHeight, 0.0f), XMFLOAT3(1.0f, 0.74f, 0.42f));
        PushRectProxy(result.lights, XMFLOAT3(center.x + extent.x * 0.12f, windowY + extent.y * 0.02f, frontZ), XMFLOAT3(windowWidth * 1.2f, 0.0f, 0.0f), XMFLOAT3(0.0f, windowHeight * 0.9f, 0.0f), XMFLOAT3(0.55f, 0.78f, 1.0f));
        PushRectProxy(result.lights, XMFLOAT3(center.x, scene.boundsMin.y + extent.y * 0.48f, center.z), XMFLOAT3(extent.x * 0.08f, 0.0f, 0.0f), XMFLOAT3(0.0f, 0.0f, extent.z * 0.08f), XMFLOAT3(1.0f, 0.82f, 0.55f));
        result.proceduralProxyCount = 3;

        result.activeLightCount = static_cast<uint32_t>(result.lights.size());
        if (result.lights.empty())
        {
            result.lights.push_back(RasterLight{});
        }
        return result;
    }
}
