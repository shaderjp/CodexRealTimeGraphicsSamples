...
PSInput VSMain(VSInput input)
{
    ...
    result.shadowPosition = mul(float4(input.position, 1.0f), g_scene.lightViewProjection);
    return result;
}
...
float ComputeShadowFactor(float4 shadowPosition, float3 normal)
{
...
float4 PSMain(PSInput input) : SV_TARGET
{
    ...
    float shadowFactor = ComputeShadowFactor(biasedShadowPosition, normal);
