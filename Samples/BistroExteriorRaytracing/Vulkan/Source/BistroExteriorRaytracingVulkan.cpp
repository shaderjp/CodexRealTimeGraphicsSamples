...
bool BistroExteriorRaytracingVulkan::QueueFamilyIndices::IsComplete() const
{
    return graphicsFamily != UINT32_MAX && presentFamily != UINT32_MAX;
}

BistroExteriorRaytracingVulkan::BistroExteriorRaytracingVulkan(uint32_t width, uint32_t height, const wchar_t* title, BistroRaytracingMode mode) :
    m_width(width),
    m_height(height),
    m_title(title),
    m_mode(mode)
...
BistroExteriorRaytracingVulkan::~BistroExteriorRaytracingVulkan()
{
    Cleanup();
}

int BistroExteriorRaytracingVulkan::Run(HINSTANCE instance, int showCommand)
{
...
LRESULT CALLBACK BistroExteriorRaytracingVulkan::WindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    auto* app = reinterpret_cast<BistroExteriorRaytracingVulkan*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
    if (message == WM_NCCREATE)
    {
        auto* createStruct = reinterpret_cast<CREATESTRUCT*>(lParam);
        app = reinterpret_cast<BistroExteriorRaytracingVulkan*>(createStruct->lpCreateParams);
...
void BistroExteriorRaytracingVulkan::InitWindow(HINSTANCE instance, int showCommand)
{
    ...
    windowClass.lpszClassName = L"BistroExteriorRaytracingVulkanWindow";
...
void BistroExteriorRaytracingVulkan::InitVulkan()
{
    CreateInstance();
    CreateSurface();
    PickPhysicalDevice();
...
void BistroExteriorRaytracingVulkan::MainLoop()
{
    MSG msg{};
...
void BistroExteriorRaytracingVulkan::Render()
{
...
void BistroExteriorRaytracingVulkan::Cleanup()
{
    if (m_device == VK_NULL_HANDLE)
    {
        return;
...
void BistroExteriorRaytracingVulkan::CreateInstance()
{
    VkApplicationInfo appInfo{};
    appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    appInfo.pApplicationName = "BistroExteriorRaytracing";
...
void BistroExteriorRaytracingVulkan::CreateSurface()
{
...
void BistroExteriorRaytracingVulkan::PickPhysicalDevice()
{
    ...
    {
        throw std::runtime_error("No suitable Vulkan device was found. BistroExteriorRaytracingVulkan requires Vulkan 1.2, swapchain support, and KHR ray tracing features.");
    }
...
void BistroExteriorRaytracingVulkan::CreateLogicalDevice()
{
...
void BistroExteriorRaytracingVulkan::LoadRayTracingFunctions()
{
    ...
    {
        throw std::runtime_error("A required Vulkan ray tracing function pointer could not be loaded.");
    }
}

void BistroExteriorRaytracingVulkan::CreateSwapChain()
{
    ...
    {
        throw std::runtime_error("The selected swapchain format does not support storage images for ray tracing output.");
    }
}

void BistroExteriorRaytracingVulkan::CreateImageViews()
{
...
void BistroExteriorRaytracingVulkan::CreateRenderPass()
{
...
void BistroExteriorRaytracingVulkan::CreateFramebuffers()
{
...
void BistroExteriorRaytracingVulkan::CreateCommandPool()
{
...
void BistroExteriorRaytracingVulkan::LoadModel()
{
    ...
    m_defaultCameraPosition = DirectX::XMFLOAT3(center.x, center.y + 3.0f, m_scene.boundsMin.z - 20.0f);
...
void BistroExteriorRaytracingVulkan::CreateOutputImages()
{
...
void BistroExteriorRaytracingVulkan::CreateSceneBuffers()
{
...
uint32_t BistroExteriorRaytracingVulkan::CreateTextureResource(const std::wstring& path, bool srgb, const uint8_t fallback[4], std::map<std::wstring, uint32_t>& cache)
{
...
void BistroExteriorRaytracingVulkan::CreateTextureImages()
{
...
void BistroExteriorRaytracingVulkan::CreateTextureSampler()
{
...
void BistroExteriorRaytracingVulkan::CreateDescriptorSetLayouts()
{
    ...
    bindings[3] = { 3, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, rtStages, nullptr };
    ...
    ThrowIfFailed(vkCreateDescriptorSetLayout(m_device, &layoutInfo, nullptr, &m_descriptorSetLayout), "Failed to create ray tracing descriptor set layout.");

    const uint32_t textureDescriptorCount = (std::max)(1u, static_cast<uint32_t>(m_scene.materials.size() * TextureSlotCount));
...
void BistroExteriorRaytracingVulkan::CreateDescriptorPool()
{
    const uint32_t textureDescriptorCount = (std::max)(1u, static_cast<uint32_t>(m_scene.materials.size() * TextureSlotCount));
...
void BistroExteriorRaytracingVulkan::CreateDescriptorSets()
{
    ...
    ThrowIfFailed(vkAllocateDescriptorSets(m_device, &allocateInfo, &m_descriptorSet), "Failed to allocate ray tracing descriptor set.");
...
void BistroExteriorRaytracingVulkan::CreateRayTracingPipeline()
{
    if (m_rtPipelineProperties.maxRayRecursionDepth < MaxTraceRecursionDepth())
    {
        throw std::runtime_error("The selected Vulkan device does not support the ray recursion depth required by this ray tracing variant.");
    }
    ...
    ThrowIfFailed(vkCreatePipelineLayout(m_device, &layoutInfo, nullptr, &m_pipelineLayout), "Failed to create ray tracing pipeline layout.");
    ...
    ThrowIfFailed(m_vkCreateRayTracingPipelinesKHR(m_device, VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_rayTracingPipeline), "Failed to create ray tracing pipeline.");

    vkDestroyShaderModule(m_device, shaderModule, nullptr);
}

void BistroExteriorRaytracingVulkan::CreateShaderBindingTables()
{
...
void BistroExteriorRaytracingVulkan::BuildAccelerationStructures()
{
...
void BistroExteriorRaytracingVulkan::CreateCommandBuffers()
{
...
void BistroExteriorRaytracingVulkan::RecordCommandBuffer(uint32_t imageIndex)
{
    ...
    vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, _countof(copyBarriers), copyBarriers);
    ...
    vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, 0, 0, nullptr, 0, nullptr, _countof(afterCopyBarriers), afterCopyBarriers);
...
void BistroExteriorRaytracingVulkan::CreateSyncObjects()
{
...
void BistroExteriorRaytracingVulkan::InitializeImGui()
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
void BistroExteriorRaytracingVulkan::ShutdownImGui()
{
...
void BistroExteriorRaytracingVulkan::BuildUI()
{
    ...
    if (m_mode == BistroRaytracingMode::GI)
    {
        ImGui::Separator();
...
void BistroExteriorRaytracingVulkan::BuildRendererStatsUI()
{
...
void BistroExteriorRaytracingVulkan::ResetLight()
{
...
void BistroExteriorRaytracingVulkan::ResetCameraView()
{
    m_camera.Reset(m_defaultCameraPosition, m_defaultCameraYaw, m_defaultCameraPitch);
}

void BistroExteriorRaytracingVulkan::ResetCameraSpeeds()
{
...
void BistroExteriorRaytracingVulkan::ResetAccumulation()
{
    m_accumulatedFrames = 0;
    m_resetAccumulationRequested = true;
}

bool BistroExteriorRaytracingVulkan::HasAccumulationStateChanged()
{
    DirectX::XMFLOAT3 cameraPosition = m_camera.GetPosition();
...
void BistroExteriorRaytracingVulkan::UpdateUniformBuffer(float)
{
    if (m_mode == BistroRaytracingMode::GI && HasAccumulationStateChanged())
    {
        ResetAccumulation();
    }
    ...
    if (m_mode == BistroRaytracingMode::GI && !m_freezeAccumulation)
    {
...
bool BistroExteriorRaytracingVulkan::IsDeviceSuitable(VkPhysicalDevice device) const
{
...
BistroExteriorRaytracingVulkan::QueueFamilyIndices BistroExteriorRaytracingVulkan::FindQueueFamilies(VkPhysicalDevice device) const
{
...
BistroExteriorRaytracingVulkan::SwapChainSupport BistroExteriorRaytracingVulkan::QuerySwapChainSupport(VkPhysicalDevice device) const
{
...
VkSurfaceFormatKHR BistroExteriorRaytracingVulkan::ChooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats) const
{
...
VkPresentModeKHR BistroExteriorRaytracingVulkan::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& presentModes) const
{
...
VkExtent2D BistroExteriorRaytracingVulkan::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) const
{
...
uint32_t BistroExteriorRaytracingVulkan::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) const
{
...
void BistroExteriorRaytracingVulkan::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, GpuBuffer& buffer)
{
    buffer.size = size;
...
void BistroExteriorRaytracingVulkan::DestroyBuffer(GpuBuffer& buffer)
{
...
void BistroExteriorRaytracingVulkan::UploadBuffer(const void* data, VkDeviceSize size, VkBufferUsageFlags usage, GpuBuffer& destination)
{
    GpuBuffer staging;
...
VkDeviceAddress BistroExteriorRaytracingVulkan::GetBufferAddress(VkBuffer buffer) const
{
...
void BistroExteriorRaytracingVulkan::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageUsageFlags usage, VkImage& image, VkDeviceMemory& memory)
{
...
VkImageView BistroExteriorRaytracingVulkan::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) const
{
...
VkCommandBuffer BistroExteriorRaytracingVulkan::BeginSingleTimeCommands() const
{
    VkCommandBufferAllocateInfo allocateInfo{};
    allocateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocateInfo.commandPool = m_commandPool;
...
void BistroExteriorRaytracingVulkan::EndSingleTimeCommands(VkCommandBuffer commandBuffer) const
{
...
void BistroExteriorRaytracingVulkan::CopyBuffer(VkBuffer src, VkBuffer dst, VkDeviceSize size) const
{
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
void BistroExteriorRaytracingVulkan::CopyBufferToImage(VkBuffer buffer, VkImage image, const Bistro::TextureData& texture) const
{
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
void BistroExteriorRaytracingVulkan::TransitionImageLayout(VkCommandBuffer commandBuffer, VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout, VkImageAspectFlags aspectMask, VkPipelineStageFlags srcStage, VkPipelineStageFlags dstStage) const
{
...
void BistroExteriorRaytracingVulkan::TransitionImageLayoutImmediate(VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout, VkImageAspectFlags aspectMask) const
{
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
std::vector<char> BistroExteriorRaytracingVulkan::ReadFile(const std::wstring& path) const
{
...
VkShaderModule BistroExteriorRaytracingVulkan::CreateShaderModule(const std::vector<char>& code) const
{
...
std::wstring BistroExteriorRaytracingVulkan::GetExecutableDirectory() const
{
...
std::wstring BistroExteriorRaytracingVulkan::ShaderFileName() const
{
    if (m_mode == BistroRaytracingMode::GI)
    {
        return L"BistroExteriorRaytracingGI.spv";
    }
    if (m_mode == BistroRaytracingMode::Shadow)
    {
        return L"BistroExteriorRaytracingShadow.spv";
    }
    return L"BistroExteriorRaytracing.spv";
}

uint32_t BistroExteriorRaytracingVulkan::MaxTraceRecursionDepth() const
{
    return m_mode == BistroRaytracingMode::GI ? 3u : (m_mode == BistroRaytracingMode::Shadow ? 2u : 1u);
}

VkDeviceSize BistroExteriorRaytracingVulkan::Align(VkDeviceSize value, VkDeviceSize alignment) const
{
    return (value + alignment - 1) & ~(alignment - 1);
}

void BistroExteriorRaytracingVulkan::ThrowIfFailed(VkResult result, const char* message) const
{
