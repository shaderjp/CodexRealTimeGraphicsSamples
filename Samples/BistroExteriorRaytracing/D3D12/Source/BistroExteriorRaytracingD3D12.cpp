#include "stdafx.h"
#include "BistroExteriorRaytracingD3D12.h"
...
namespace
{
    ...
    std::wstring RaytracingTierName(D3D12_RAYTRACING_TIER tier)
    {
...
BistroExteriorRaytracingD3D12::BistroExteriorRaytracingD3D12(UINT width, UINT height, std::wstring name, BistroRaytracingMode mode) :
...
void BistroExteriorRaytracingD3D12::OnInit()
{
    LoadPipeline();
    LoadAssets();
...
void BistroExteriorRaytracingD3D12::LoadPipeline()
{
    ...
    {
        throw std::runtime_error("This GPU/driver does not support DirectX Raytracing. BistroExteriorRaytracingD3D12 has no raster fallback.");
    }
...
void BistroExteriorRaytracingD3D12::LoadAssets()
{
    m_scene = Bistro::LoadScene(Bistro::FindAssetRoot());
    if (m_scene.draws.empty())
    {
        throw std::runtime_error("BistroExterior.fbx did not produce ray tracing geometries.");
    }
    ...
    m_defaultCameraPosition = XMFLOAT3(center.x, center.y + 3.0f, m_scene.boundsMin.z - 20.0f);
...
void BistroExteriorRaytracingD3D12::CreateDescriptorHeap()
{
    m_descriptorCount = DescriptorTextureBase + static_cast<UINT>(m_scene.materials.size()) * TextureSlotCount;
...
void BistroExteriorRaytracingD3D12::CreateOutputResources()
{
    ...
    ThrowIfFailed(m_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE, &outputDesc, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&m_raytracingOutput)));
    ...
    m_accumulationOutput->SetName(L"RaytracingAccumulation");
    ...
    m_device->CreateUnorderedAccessView(m_raytracingOutput.Get(), nullptr, &outputUav, CpuDescriptor(DescriptorOutputUav));
...
void BistroExteriorRaytracingD3D12::CreateGlobalRootSignature()
{
    CD3DX12_DESCRIPTOR_RANGE uavRange;
    uavRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 2, 0, 0);
    CD3DX12_DESCRIPTOR_RANGE sceneBufferRange;
    sceneBufferRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 4, 1, 0);
    CD3DX12_DESCRIPTOR_RANGE textureRange;
    textureRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, static_cast<UINT>(m_scene.materials.size()) * TextureSlotCount, 0, 1);
...
void BistroExteriorRaytracingD3D12::CreateSceneBuffers()
{
...
UINT BistroExteriorRaytracingD3D12::CreateTextureResource(const std::wstring& path, bool srgb, const uint8_t fallback[4], std::map<std::wstring, UINT>& cache)
{
...
void BistroExteriorRaytracingD3D12::CreateTextures()
{
...
void BistroExteriorRaytracingD3D12::CreateRaytracingStateObject()
{
    ...
    shaderConfig->Config(96, sizeof(XMFLOAT2));
...
void BistroExteriorRaytracingD3D12::BuildAccelerationStructures()
{
...
void BistroExteriorRaytracingD3D12::CreateShaderTables()
{
    ...
    auto createTable = [&](const wchar_t* name, std::initializer_list<const wchar_t*> exports, ShaderTableInfo& table)
    {
        ...
        for (const wchar_t* exportName : exports)
        {
            ...
            if (!identifier)
            {
                throw std::runtime_error("Failed to resolve a ray tracing shader identifier.");
            }
...
void BistroExteriorRaytracingD3D12::OnUpdate()
{
...
void BistroExteriorRaytracingD3D12::UpdateConstantBuffer(float)
{
    ...
    constants.debugOptions = XMFLOAT4(static_cast<float>(m_debugViewMode), m_debugNormalMapYFlip ? 1.0f : 0.0f, m_shadowEnabled ? 1.0f : 0.0f, 0.0f);
    constants.skyColor = XMFLOAT4(m_skyColor[0], m_skyColor[1], m_skyColor[2], m_skyIntensity);
    constants.rayOptions = XMFLOAT4(m_rayTMin, m_rayTMax, m_giStrength, 0.0f);
...
void BistroExteriorRaytracingD3D12::OnRender()
{
    BuildUI();
    PopulateCommandList();
...
void BistroExteriorRaytracingD3D12::PopulateCommandList()
{
    ThrowIfFailed(m_commandAllocator->Reset());
    ThrowIfFailed(m_commandList->Reset(m_commandAllocator.Get(), nullptr));
...
void BistroExteriorRaytracingD3D12::DispatchRays()
{
    ...
    m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::UAV(m_raytracingOutput.Get()));
}

void BistroExteriorRaytracingD3D12::CopyOutputToBackBuffer()
{
    ...
    m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_raytracingOutput.Get(), D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS));
}

void BistroExteriorRaytracingD3D12::BuildUI()
{
    ...
    ImGui::Combo("Debug View", &m_debugViewMode, debugModes, _countof(debugModes));
    ImGui::Checkbox("Normal Map Y Flip", &m_debugNormalMapYFlip);
    ImGui::SliderFloat("Ray Bias / TMin", &m_rayTMin, 0.001f, 0.25f, "%.3f");
    ImGui::ColorEdit3("Sky Color", m_skyColor);
    ImGui::SliderFloat("Sky Intensity", &m_skyIntensity, 0.0f, 10.0f, "%.2f");
...
void BistroExteriorRaytracingD3D12::BuildRendererStatsUI()
{
    ...
    ImGui::Text("DXR Tier: %ls", RaytracingTierName(m_raytracingTier).c_str());
...
void BistroExteriorRaytracingD3D12::OnWindowMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void BistroExteriorRaytracingD3D12::OnDestroy()
{
    ShutdownImGui();
    WaitForPreviousFrame();
...
void BistroExteriorRaytracingD3D12::WaitForPreviousFrame()
{
...
void BistroExteriorRaytracingD3D12::InitializeImGui()
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
void BistroExteriorRaytracingD3D12::ShutdownImGui()
{
...
void BistroExteriorRaytracingD3D12::ResetLight()
{
...
void BistroExteriorRaytracingD3D12::ResetCameraView()
{
    m_camera.Reset(m_defaultCameraPosition, m_defaultCameraYaw, m_defaultCameraPitch);
}

void BistroExteriorRaytracingD3D12::ResetCameraSpeeds()
{
...
void BistroExteriorRaytracingD3D12::ResetAccumulation()
{
    m_accumulatedFrames = 0;
    m_resetAccumulationRequested = true;
}

bool BistroExteriorRaytracingD3D12::HasAccumulationStateChanged()
{
    XMFLOAT3 cameraPosition = m_camera.GetPosition();
    XMFLOAT4 cameraAndYaw(cameraPosition.x, cameraPosition.y, cameraPosition.z, m_camera.GetYawRadians() + m_camera.GetPitchRadians());
    XMFLOAT4 lighting(m_lightDirection[0], m_lightDirection[1], m_lightDirection[2], m_lightIntensity + m_giStrength + static_cast<float>(m_debugViewMode));
    const bool changed =
...
ComPtr<ID3D12Resource> BistroExteriorRaytracingD3D12::CreateDefaultBuffer(const void* data, UINT64 size, D3D12_RESOURCE_FLAGS flags, D3D12_RESOURCE_STATES finalState, const wchar_t* name)
{
...
ComPtr<ID3D12Resource> BistroExteriorRaytracingD3D12::CreateUploadBuffer(const void* data, UINT64 size, const wchar_t* name)
{
    ComPtr<ID3D12Resource> resource;
...
ComPtr<ID3D12Resource> BistroExteriorRaytracingD3D12::CreateUavBuffer(UINT64 size, D3D12_RESOURCE_STATES initialState, const wchar_t* name)
{
    ComPtr<ID3D12Resource> resource;
...
D3D12_CPU_DESCRIPTOR_HANDLE BistroExteriorRaytracingD3D12::CpuDescriptor(UINT index) const
{
    return CD3DX12_CPU_DESCRIPTOR_HANDLE(m_descriptorHeap->GetCPUDescriptorHandleForHeapStart(), index, m_descriptorSize);
}

D3D12_GPU_DESCRIPTOR_HANDLE BistroExteriorRaytracingD3D12::GpuDescriptor(UINT index) const
{
    return CD3DX12_GPU_DESCRIPTOR_HANDLE(m_descriptorHeap->GetGPUDescriptorHandleForHeapStart(), index, m_descriptorSize);
}

std::wstring BistroExteriorRaytracingD3D12::ShaderFileName() const
{
    if (m_mode == BistroRaytracingMode::GI)
    {
        return L"BistroExteriorRaytracingGI.lib.cso";
    }
    if (m_mode == BistroRaytracingMode::Shadow)
    {
        return L"BistroExteriorRaytracingShadow.lib.cso";
    }
    return L"BistroExteriorRaytracing.lib.cso";
}

UINT BistroExteriorRaytracingD3D12::MaxTraceRecursionDepth() const
{
    return m_mode == BistroRaytracingMode::GI ? 3u : (m_mode == BistroRaytracingMode::Shadow ? 2u : 1u);
}

UINT BistroExteriorRaytracingD3D12::Align(UINT value, UINT alignment)
{
    return (value + alignment - 1) & ~(alignment - 1);
}

void BistroExteriorRaytracingD3D12::AllocateImGuiDescriptor(ImGui_ImplDX12_InitInfo* info, D3D12_CPU_DESCRIPTOR_HANDLE* outCpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE* outGpuHandle)
{
    auto* sample = static_cast<BistroExteriorRaytracingD3D12*>(info->UserData);
...
void BistroExteriorRaytracingD3D12::FreeImGuiDescriptor(ImGui_ImplDX12_InitInfo*, D3D12_CPU_DESCRIPTOR_HANDLE, D3D12_GPU_DESCRIPTOR_HANDLE)
{
}
