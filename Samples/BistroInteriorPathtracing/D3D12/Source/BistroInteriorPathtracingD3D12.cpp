#include "stdafx.h"
#include "BistroInteriorPathtracingD3D12.h"
...
BistroInteriorPathtracingD3D12::BistroInteriorPathtracingD3D12(UINT width, UINT height, std::wstring name, BistroPathtracingMode mode) :
...
void BistroInteriorPathtracingD3D12::OnInit()
{
    LoadPipeline();
    LoadAssets();
...
void BistroInteriorPathtracingD3D12::LoadPipeline()
{
    ...
    {
        throw std::runtime_error("This GPU/driver does not support DirectX Pathtracing. BistroInteriorPathtracingD3D12 has no raster fallback.");
    }
...
void BistroInteriorPathtracingD3D12::LoadAssets()
{
    ...
    m_defaultCameraPosition = XMFLOAT3(center.x, m_scene.boundsMin.y + extent.y * 0.38f, center.z - extent.z * 0.28f);
    m_defaultCameraYaw = 0.0f;
    m_defaultCameraPitch = XMConvertToRadians(-1.5f);
    ResetCameraView();
    ResetCameraSpeeds();
...
void BistroInteriorPathtracingD3D12::CreateDescriptorHeap()
{
...
void BistroInteriorPathtracingD3D12::CreateOutputResources()
{
...
void BistroInteriorPathtracingD3D12::CreateGlobalRootSignature()
{
...
void BistroInteriorPathtracingD3D12::CreateSceneBuffers()
{
...
UINT BistroInteriorPathtracingD3D12::CreateTextureResource(const std::wstring& path, bool srgb, const uint8_t fallback[4], std::map<std::wstring, UINT>& cache)
{
...
void BistroInteriorPathtracingD3D12::CreateTextures()
{
...
void BistroInteriorPathtracingD3D12::CreatePathtracingStateObject()
{
...
void BistroInteriorPathtracingD3D12::CreateRestirReusePipeline()
{
...
void BistroInteriorPathtracingD3D12::CreateDenoisePipeline()
{
    const std::wstring exeDir = GetAssetFullPath(L"");
    byte* shaderData = nullptr;
    UINT shaderSize = 0;
...
void BistroInteriorPathtracingD3D12::BuildAccelerationStructures()
{
...
void BistroInteriorPathtracingD3D12::CreateShaderTables()
{
...
void BistroInteriorPathtracingD3D12::OnUpdate()
{
...
void BistroInteriorPathtracingD3D12::UpdateConstantBuffer(float)
{
...
void BistroInteriorPathtracingD3D12::OnRender()
{
    BuildUI();
    PopulateCommandList();
...
void BistroInteriorPathtracingD3D12::PopulateCommandList()
{
    ThrowIfFailed(m_commandAllocator->Reset());
    ThrowIfFailed(m_commandList->Reset(m_commandAllocator.Get(), nullptr));
...
void BistroInteriorPathtracingD3D12::DispatchRays()
{
...
void BistroInteriorPathtracingD3D12::RunRestirReusePass()
{
...
void BistroInteriorPathtracingD3D12::RunDenoisePass()
{
...
void BistroInteriorPathtracingD3D12::CopyOutputToBackBuffer()
{
...
void BistroInteriorPathtracingD3D12::BuildUI()
{
...
void BistroInteriorPathtracingD3D12::BuildRendererStatsUI()
{
...
void BistroInteriorPathtracingD3D12::ResizeOutput(UINT width, UINT height)
{
...
void BistroInteriorPathtracingD3D12::OnKeyDown(UINT8 key)
{
    if (key == VK_SPACE)
    {
        ResetCameraView();
...
void BistroInteriorPathtracingD3D12::OnWindowMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void BistroInteriorPathtracingD3D12::OnDestroy()
{
    ShutdownImGui();
    WaitForPreviousFrame();
...
void BistroInteriorPathtracingD3D12::WaitForPreviousFrame()
{
...
void BistroInteriorPathtracingD3D12::InitializeImGui()
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
void BistroInteriorPathtracingD3D12::ShutdownImGui()
{
...
void BistroInteriorPathtracingD3D12::ResetLight()
{
...
void BistroInteriorPathtracingD3D12::ResetCameraView()
{
    m_camera.Reset(m_defaultCameraPosition, m_defaultCameraYaw, m_defaultCameraPitch);
}

void BistroInteriorPathtracingD3D12::ResetCameraSpeeds()
{
...
void BistroInteriorPathtracingD3D12::ResetAccumulation()
{
    m_accumulatedFrames = 0;
    m_resetAccumulationRequested = true;
}

bool BistroInteriorPathtracingD3D12::HasAccumulationStateChanged()
{
    XMFLOAT3 cameraPosition = m_camera.GetPosition();
...
ComPtr<ID3D12Resource> BistroInteriorPathtracingD3D12::CreateDefaultBuffer(const void* data, UINT64 size, D3D12_RESOURCE_FLAGS flags, D3D12_RESOURCE_STATES finalState, const wchar_t* name)
{
...
ComPtr<ID3D12Resource> BistroInteriorPathtracingD3D12::CreateUploadBuffer(const void* data, UINT64 size, const wchar_t* name)
{
    ComPtr<ID3D12Resource> resource;
...
ComPtr<ID3D12Resource> BistroInteriorPathtracingD3D12::CreateUavBuffer(UINT64 size, D3D12_RESOURCE_STATES initialState, const wchar_t* name)
{
    ComPtr<ID3D12Resource> resource;
...
D3D12_CPU_DESCRIPTOR_HANDLE BistroInteriorPathtracingD3D12::CpuDescriptor(UINT index) const
{
    return CD3DX12_CPU_DESCRIPTOR_HANDLE(m_descriptorHeap->GetCPUDescriptorHandleForHeapStart(), index, m_descriptorSize);
}

D3D12_GPU_DESCRIPTOR_HANDLE BistroInteriorPathtracingD3D12::GpuDescriptor(UINT index) const
{
    return CD3DX12_GPU_DESCRIPTOR_HANDLE(m_descriptorHeap->GetGPUDescriptorHandleForHeapStart(), index, m_descriptorSize);
}

std::wstring BistroInteriorPathtracingD3D12::ShaderFileName() const
{
    if (m_mode == BistroPathtracingMode::ReSTIR)
    {
        return L"BistroInteriorPathtracingReSTIR.lib.cso";
    }
    if (m_mode == BistroPathtracingMode::ReSTIRDI)
    {
        return L"BistroInteriorPathtracingReSTIRDI.lib.cso";
    }
    return L"BistroInteriorPathtracing.lib.cso";
}

std::wstring BistroInteriorPathtracingD3D12::RestirReuseShaderFileName() const
{
    return L"BistroInteriorPathtracingReSTIRResolve.cso";
}

std::wstring BistroInteriorPathtracingD3D12::DenoiseShaderFileName() const
{
    return L"BistroInteriorPathtracingDenoise.cso";
}

UINT BistroInteriorPathtracingD3D12::MaxTraceRecursionDepth() const
{
    return 1u;
}

UINT BistroInteriorPathtracingD3D12::Align(UINT value, UINT alignment)
{
    return (value + alignment - 1) & ~(alignment - 1);
}

void BistroInteriorPathtracingD3D12::AllocateImGuiDescriptor(ImGui_ImplDX12_InitInfo* info, D3D12_CPU_DESCRIPTOR_HANDLE* outCpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE* outGpuHandle)
{
    auto* sample = static_cast<BistroInteriorPathtracingD3D12*>(info->UserData);
...
void BistroInteriorPathtracingD3D12::FreeImGuiDescriptor(ImGui_ImplDX12_InitInfo*, D3D12_CPU_DESCRIPTOR_HANDLE, D3D12_GPU_DESCRIPTOR_HANDLE)
{
}
